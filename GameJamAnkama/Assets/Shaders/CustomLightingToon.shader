...
Shader "Custom/CustomLightingToon"
{
	...
	SubShader
	{
		...
		inline half4 LightingStandardCustomLighting( inout SurfaceOutputCustomLightingCustom s, half3 viewDir, UnityGI gi )
		{
			...
			c.rgb = ( ( _DiffuseColor * ( tex2D( _Diffuse, uv_Diffuse ) * tex2D( _ToonRamp, temp_cast_0 ) ) * ( _LightColor0 * float4( ( ase_indirectDiffuse + temp_cast_2 ) , 0.0 ) ) ) + ( saturate( ( ( ase_lightAtten * temp_output_3_0 ) * pow( ( 1.0 - saturate( ( dot( i.worldNormal , i.viewDir ) + _RimOffset ) ) ) , _RimPower ) ) ) * ( _RimColor * _LightColor0 ) ) ).rgb;
			c.a = 1;
			return c;
		}
